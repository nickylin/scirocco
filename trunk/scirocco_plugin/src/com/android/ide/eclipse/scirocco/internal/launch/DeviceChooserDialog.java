...
import com.android.ide.eclipse.scirocco.internal.sdk.AdtConsoleSdkLog;
...
public class DeviceChooserDialog extends Dialog implements IDeviceChangeListener {
    ...
    private Button mAllDeviceRadioButton;
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        //TODO 全機種テスト用のラジオボタンを生成
        mAllDeviceRadioButton = new Button(top, SWT.RADIO);
        mAllDeviceRadioButton.setText("All Connected Android device");
        mAllDeviceRadioButton.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
            	//TODO　全機種テストを行うフラグをたてる
            	AdtPlugin.setAllDevicesSelected(true);
                mDeviceTable.setEnabled(false);
                mPreferredAvdSelector.setEnabled(true);
//                if (selectAllDevice) {
...
//                }
                ...
                mResponse.setDeviceToUse(devices[0]);
                Button okButton = getButton(IDialogConstants.OK_ID);
                okButton.setEnabled(true);
            }
        });
//        mAllDeviceRadioButton.setSelection(true);
        ...
        mDeviceRadioButton.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
            	AdtPlugin.setAllDevicesSelected(false);
                mDeviceTable.setEnabled(false);
    ...
    private void updateDefaultSelection() {
        if (mDeviceTable.getSelectionCount() == 0) {
            //TODO yamada bridgeのmAdbOsLocationで、adbのディレクトリを取得できる
            AndroidDebugBridge bridge = AndroidDebugBridge.getBridge();
            IDevice[] devices = bridge.getDevices();
