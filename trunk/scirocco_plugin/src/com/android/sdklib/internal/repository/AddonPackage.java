...
public class AddonPackage extends Package
    ...
    AddonPackage(SdkSource source, Node packageNode, String nsUri, Map<String,String> licenses) {
        ...
        mName     = XmlParserUtils.getXmlString(packageNode, RepoConstants.NODE_NAME);
        int apiLevel = XmlParserUtils.getXmlInt   (packageNode, RepoConstants.NODE_API_LEVEL, 0);
        String codeName = XmlParserUtils.getXmlString(packageNode, RepoConstants.NODE_CODENAME);
        if (codeName.length() == 0) {
            codeName = null;
        }
        mVersion = new AndroidVersion(apiLevel, codeName);

        mLibs = parseLibs(XmlParserUtils.getFirstChild(packageNode, RepoConstants.NODE_LIBS));
    }
        ...
        if (libsNode != null) {
            ...
            for(Node child = libsNode.getFirstChild();
                ...
                if (child.getNodeType() == Node.ELEMENT_NODE &&
                        ...
                        RepoConstants.NODE_LIB.equals(child.getLocalName())) {
    ...
    private Lib parseLib(Node libNode) {
        return new Lib(XmlParserUtils.getXmlString(libNode, RepoConstants.NODE_NAME),
                       XmlParserUtils.getXmlString(libNode, RepoConstants.NODE_DESCRIPTION));
    }
