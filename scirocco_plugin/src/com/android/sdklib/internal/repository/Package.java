...
public abstract class Package implements IDescription, Comparable<Package> {
    ...
    Package(SdkSource source, Node packageNode, String nsUri, Map<String,String> licenses) {
        mSource = source;
        mRevision    = XmlParserUtils.getXmlInt   (packageNode, RepoConstants.NODE_REVISION, 0);
        mDescription = XmlParserUtils.getXmlString(packageNode, RepoConstants.NODE_DESCRIPTION);
        mDescUrl     = XmlParserUtils.getXmlString(packageNode, RepoConstants.NODE_DESC_URL);
        mReleaseNote = XmlParserUtils.getXmlString(packageNode, RepoConstants.NODE_RELEASE_NOTE);
        mReleaseUrl  = XmlParserUtils.getXmlString(packageNode, RepoConstants.NODE_RELEASE_URL);
        mObsolete    = XmlParserUtils.getOptionalXmlString(
                                                   packageNode, RepoConstants.NODE_OBSOLETE);

        mLicense  = parseLicense(packageNode, licenses);
        mArchives = parseArchives(XmlParserUtils.getFirstChild(
                                  packageNode, RepoConstants.NODE_ARCHIVES));
    }
    ...
    private String parseLicense(Node packageNode, Map<String, String> licenses) {
        Node usesLicense = XmlParserUtils.getFirstChild(
                                            packageNode, RepoConstants.NODE_USES_LICENSE);
        if (usesLicense != null) {
            Node ref = usesLicense.getAttributes().getNamedItem(RepoConstants.ATTR_REF);
            if (ref != null) {
        ...
        if (archivesNode != null) {
            ...
            for(Node child = archivesNode.getFirstChild();
                ...
                if (child.getNodeType() == Node.ELEMENT_NODE &&
                        ...
                        RepoConstants.NODE_ARCHIVE.equals(child.getLocalName())) {
    ...
    private Archive parseArchive(Node archiveNode) {
        Archive a = new Archive(
                    this,
                    (Os)   XmlParserUtils.getEnumAttribute(archiveNode, RepoConstants.ATTR_OS,
                            Os.values(), null),
                    (Arch) XmlParserUtils.getEnumAttribute(archiveNode, RepoConstants.ATTR_ARCH,
                            Arch.values(), Arch.ANY),
                    XmlParserUtils.getXmlString(archiveNode, RepoConstants.NODE_URL),
                    XmlParserUtils.getXmlLong  (archiveNode, RepoConstants.NODE_SIZE, 0),
                    XmlParserUtils.getXmlString(archiveNode, RepoConstants.NODE_CHECKSUM)
                );
